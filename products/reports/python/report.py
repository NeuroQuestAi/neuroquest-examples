...
async def login():
    async with aiohttp.ClientSession() as session:
        async with session.post(
            "https://api-reports.neuroquest.ai/api/v1/auth/login",
...
async def logout():
    async with aiohttp.ClientSession() as session:
        async with session.get(
            f"https://api-reports.neuroquest.ai/api/v1/auth/logout?email={os.getenv('NQ_USER')}",
            headers={
...
async def create():
    async with aiohttp.ClientSession() as session:
        async with session.post(
            "https://api-reports.neuroquest.ai/api/v1/report/create",
            headers={
                "Content-Type": "application/json",
                "token": os.getenv("NQ_TOKEN"),
            },
...
async def main():
    parser = argparse.ArgumentParser(description="Example using the Reports API")
    ...
    parser.add_argument("--create", action="store_true", help="Create an assessment")
...
if __name__ == "__main__":
    if (
        ...
        or not os.getenv("NQ_POWER_SKILLS_ANALYSIS_DOC_ID")
        or not os.getenv("NQ_REPORT_TEMPLATE")
        or not os.getenv("NQ_PROFILE_PICTURE")
    ):
        print(
            "Environment variables not defined! Check: NQ_USER | NQ_PASSWORD | NQ_TOKEN | NQ_POWER_SKILLS_ANALYSIS_DOC_ID | NQ_REPORT_TEMPLATE | NQ_PROFILE_PICTURE"
        )
    else:
        asyncio.run(main())
